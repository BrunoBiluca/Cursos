...
void particionar(int esq, int dir,
	int *i, int *j, TipoVetor A) {
	TipoItem x, w;
	*i = esq; *j = dir;
	x = A[(*i + *j) / 2];
	/* obtem o pivo x */
...
void ordenar(int esq, int dir, TipoVetor A) {
	int i, j;
...
void quicksort(TipoVetor a, int n) {
	ordenar(0, n, a);
}

//   PARA ARQUIVOS GRANDES

void particionar2(int esq, int dir,
	int *i, int *j, TipoVetor A, VetorIndices Ind) {
	TipoItem x, w;
	int y;
	...
	*i = esq; *j = dir;
	...
	do {
		...
		if (*i <= *j) {
			...
			y = Ind[*i].indice; Ind[*i].indice = Ind[*j].indice; Ind[*j].indice = y;
			(*i)++; (*j)--;
		}
	} while (*i <= *j);
}

void ordenar2(int esq, int dir, TipoVetor A, VetorIndices Ind) {
	int i, j;
	particionar2(esq, dir, &i, &j, A, Ind);
	if (esq < j) ordenar2(esq, j, A, Ind);
	if (i < dir) ordenar2(i, dir, A, Ind);
}

void quicksort2(TipoVetor a, VetorIndices Ind, int n) {
	ordenar2(0, n, a, Ind);
}
			...
			case 5:{
				...
				switch(opcao2) {
					case 1:{
						...
						fp2 = fopen("Saida_n200000p.txt","w");
						n = 199999;
						for(i = 0;i<= n; i++) {
							fscanf(fp, "\n%d\n", &A[i].Chave);
						}
					...
					case 2:{
						...
						fp2 = fopen("Saida_n200000g.txt","w");	
						n = 199999;
						for(i = 0;i<= n; i++) {
							Indices[i].indice = i;
							fscanf(fp, "\n%d\n", &A2[i].Chave2);
